...
class XOR(BooleanFunction):
    ...
    @classmethod
    def tseytin_unroll(cls,gate_labels,arg_labels):
        if len(arg_labels) == 0: # empty gate (e.g. XOR() ) => assert unsat using this gate
            ...
            clauses = cls.tseytin_formula(gate_labels[0], arg_labels[0], arg_labels[1])

            # afterward each node uses the previous + the next arg
            for i in range(1,len(gate_labels)):
                clauses += cls.tseytin_formula(
                    gate_labels[i], gate_labels[i-1], arg_labels[i+1]
                )
            return clauses
...
class XNOR(BooleanFunction):
    ...
    def generate_c(self, cache, array_name):
        return "(!(" + " ^ ".join(cache[arg] for arg in self.args) + "))"
    def _generate_VHDL(self, cache, array_name):
    ...
    def tseytin_unroll(cls,gate_labels,arg_labels):
        if len(arg_labels) == 0: # empty gate (e.g. XOR() ) => assert unsat using this gate
            ...
            return cls.tseytin_formula(gate_labels[0], arg_labels[0], arg_labels[1])
        
        else:
            # initial node uses 2 args
            clauses = XOR.tseytin_formula(gate_labels[0], arg_labels[0], arg_labels[1])
...
class NAND(BooleanFunction):
    ...
    def tseytin_unroll(cls,gate_labels,arg_labels):
        if len(arg_labels) == 0: # empty gate (e.g. AND() ) => assert unsat using this gate
            ...
            # initial node uses 2 args
            clauses = AND.tseytin_formula(gate_labels[0], arg_labels[0], arg_labels[1])
...
class NOR(BooleanFunction):
    ...
    def tseytin_unroll(cls,gate_labels,arg_labels):
        if len(arg_labels) == 0: # empty gate (e.g. AND() ) => assert unsat using this gate
            ...
            # initial node uses 2 args
            clauses = OR.tseytin_formula(gate_labels[0], arg_labels[0], arg_labels[1])
