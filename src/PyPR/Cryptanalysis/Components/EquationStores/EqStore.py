...
class EqStore:
    def __init__(self, comb_to_idx):        
        ...
        # ensure an index for 1
        # if tuple() not in comb_to_idx:
    ...
    def insert_equation(self, 
        ...
        if type(equation) == np.ndarray and equation.shape == (self.num_vars,):
            ...
            if translate_ANF:
                equation = equation.translate_ANF()

            coef_vector = np.zeros([self.num_vars], dtype=np.uint8)
            #const_val = extra_const
            coef_vector[self.comb_to_idx[tuple()]] = extra_const
